...
<div class="grid grid-cols-2 md:grid-cols-3 gap-4 content-center px-64">
    {#each linkLibrary as image}
        <button
            type="button"
            class="relative group cursor-pointer overflow-hidden rounded-lg shadow-md bg-black"
            onclick={() => openModal(image)}
        >
            <img
                src={image.visualLink}
                alt={image.description}
                class="w-full h-64 object-cover transition-transform duration-300 group-hover:scale-140"
            />
